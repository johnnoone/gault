...
def test_add_to_set(subtests, context):
    with subtests.test():
        acc = AddToSet("$var")
        assert acc.compile_expression(context=context) == {"$addToSet": "$var"}

    with subtests.test():
        acc = AddToSet(Field("foo.bar"))
        assert acc.compile_expression(context=context) == {"$addToSet": "$foo.bar"}

    with subtests.test():
        acc = AddToSet(MyModel.id)
        assert acc.compile_expression(context=context) == {"$addToSet": "$id"}


def test_avg(subtests, context):
    with subtests.test():
        acc = Avg("$var")
        assert acc.compile_expression(context=context) == {"$avg": "$var"}

    with subtests.test():
        acc = Avg(Field("foo.bar"))
        assert acc.compile_expression(context=context) == {"$avg": "$foo.bar"}

    with subtests.test():
        acc = Avg(MyModel.id)
        assert acc.compile_expression(context=context) == {"$avg": "$id"}


def test_bottom(subtests, context):
    with subtests.test():
        acc = Bottom(sort_by={"score": 1}, output="$var")
        assert acc.compile_expression(context=context) == {
...
def test_count(subtests, context):
    with subtests.test():
        acc = Count()
        assert acc.compile_expression(context=context) == {"$count": {}}


def test_first(subtests, context):
    with subtests.test():
        acc = First("$var")
        assert acc.compile_expression(context=context) == {"$first": "$var"}

    with subtests.test():
        acc = First(Field("foo.bar"))
        assert acc.compile_expression(context=context) == {"$first": "$foo.bar"}

    with subtests.test():
        acc = First(MyModel.id)
        assert acc.compile_expression(context=context) == {"$first": "$id"}


def test_first_n(subtests, context):
    with subtests.test():
        acc = FirstN(input="$var", n=3)
        assert acc.compile_expression(context=context) == {
            "$firstN": {"input": "$var", "n": 3}
        }

    with subtests.test():
        acc = FirstN(input=Field("foo.bar"), n=5)
        assert acc.compile_expression(context=context) == {
            "$firstN": {"input": "$foo.bar", "n": 5}
        }

    with subtests.test():
        acc = FirstN(input=MyModel.id, n=10)
        assert acc.compile_expression(context=context) == {
            "$firstN": {"input": "$id", "n": 10}
        }
...
def test_last(subtests, context):
    with subtests.test():
        acc = Last("$var")
        assert acc.compile_expression(context=context) == {"$last": "$var"}

    with subtests.test():
        acc = Last(Field("foo.bar"))
        assert acc.compile_expression(context=context) == {"$last": "$foo.bar"}

    with subtests.test():
        acc = Last(MyModel.id)
        assert acc.compile_expression(context=context) == {"$last": "$id"}


def test_last_n(subtests, context):
    with subtests.test():
        acc = LastN(input="$var", n=3)
        assert acc.compile_expression(context=context) == {
            "$lastN": {"input": "$var", "n": 3}
        }

    with subtests.test():
        acc = LastN(input=Field("foo.bar"), n=5)
        assert acc.compile_expression(context=context) == {
            "$lastN": {"input": "$foo.bar", "n": 5}
        }

    with subtests.test():
        acc = LastN(input=MyModel.id, n=10)
        assert acc.compile_expression(context=context) == {
            "$lastN": {"input": "$id", "n": 10}
        }
...
def test_max(subtests, context):
    with subtests.test():
        acc = Max("$var")
        assert acc.compile_expression(context=context) == {"$max": "$var"}

    with subtests.test():
        acc = Max(Field("foo.bar"))
        assert acc.compile_expression(context=context) == {"$max": "$foo.bar"}

    with subtests.test():
        acc = Max(MyModel.id)
        assert acc.compile_expression(context=context) == {"$max": "$id"}


def test_max_n(subtests, context):
    with subtests.test():
        acc = MaxN(input="$var", n=3)
        assert acc.compile_expression(context=context) == {
            "$maxN": {"input": "$var", "n": 3}
        }

    with subtests.test():
        acc = MaxN(input=Field("foo.bar"), n=5)
        assert acc.compile_expression(context=context) == {
            "$maxN": {"input": "$foo.bar", "n": 5}
        }

    with subtests.test():
        acc = MaxN(input=MyModel.id, n=10)
        assert acc.compile_expression(context=context) == {
            "$maxN": {"input": "$id", "n": 10}
        }
...
def test_merge_objects(subtests, context):
    with subtests.test():
        ...
        assert acc.compile_expression(context=context) == {"$mergeObjects": "$var"}

    with subtests.test():
        acc = MergeObjects(Field("foo.bar"))
        assert acc.compile_expression(context=context) == {"$mergeObjects": "$foo.bar"}

    with subtests.test():
        acc = MergeObjects(MyModel.id)
        assert acc.compile_expression(context=context) == {"$mergeObjects": "$id"}


def test_min(subtests, context):
    with subtests.test():
        acc = Min("$var")
        assert acc.compile_expression(context=context) == {"$min": "$var"}

    with subtests.test():
        acc = Min(Field("foo.bar"))
        assert acc.compile_expression(context=context) == {"$min": "$foo.bar"}

    with subtests.test():
        acc = Min(MyModel.id)
        assert acc.compile_expression(context=context) == {"$min": "$id"}


def test_min_n(subtests, context):
    with subtests.test():
        acc = MinN(input="$var", n=3)
        assert acc.compile_expression(context=context) == {
            "$minN": {"input": "$var", "n": 3}
        }

    with subtests.test():
        acc = MinN(input=Field("foo.bar"), n=5)
        assert acc.compile_expression(context=context) == {
            "$minN": {"input": "$foo.bar", "n": 5}
        }

    with subtests.test():
        acc = MinN(input=MyModel.id, n=10)
        assert acc.compile_expression(context=context) == {
            "$minN": {"input": "$id", "n": 10}
        }
...
def test_push(subtests, context):
    with subtests.test():
        acc = Push("$var")
        assert acc.compile_expression(context=context) == {"$push": "$var"}

    with subtests.test():
        acc = Push(Field("foo.bar"))
        assert acc.compile_expression(context=context) == {"$push": "$foo.bar"}

    with subtests.test():
        acc = Push(MyModel.id)
        assert acc.compile_expression(context=context) == {"$push": "$id"}


def test_std_dev_pop(subtests, context):
    with subtests.test():
        acc = StdDevPop(input="$var")
        assert acc.compile_expression(context=context) == {"$stdDevPop": "$var"}

    with subtests.test():
        acc = StdDevPop(input=Field("foo.bar"))
        assert acc.compile_expression(context=context) == {"$stdDevPop": "$foo.bar"}

    with subtests.test():
        acc = StdDevPop(input=MyModel.id)
        assert acc.compile_expression(context=context) == {"$stdDevPop": "$id"}
...
def test_sum(subtests, context):
    with subtests.test():
        acc = Sum("$var")
        assert acc.compile_expression(context=context) == {"$sum": "$var"}

    with subtests.test():
        acc = Sum(Field("foo.bar"))
        assert acc.compile_expression(context=context) == {"$sum": "$foo.bar"}

    with subtests.test():
        acc = Sum(MyModel.id)
        assert acc.compile_expression(context=context) == {"$sum": "$id"}
